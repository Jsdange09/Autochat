...
Combines Autochat (friend) + MLOps layer (you).

FIXES APPLIED:
  [FIX-1] load_dotenv() called at the very top — this is what was causing
           BOTH the W&B and Arize errors. Keys were in .env but never loaded.
  [FIX-2] Imports reordered so dotenv loads before any os.getenv() call.
"""

# ── [FIX-1] Load .env FIRST, before any other import that reads env vars ──
import os
from dotenv import load_dotenv
load_dotenv()   # ← This single line was missing. It reads .env into os.environ.
...
model = ModelCatalog().load_model("bling-phi-3-gguf", temperature=0.2, sample=False)

# W&B run — now works because load_dotenv() already populated WANDB_API_KEY
init_run()
...
CONFIDENCE_SCORE = Gauge("confidence_score",         "Model confidence score")
EMBEDDING_AGE    = Gauge("embedding_age_days",       "Simulated embedding age")
UPDATE_TRIGGER   = Gauge("update_trigger_flag",      "1 if repair needed")
...
KB_VERSION       = Gauge("knowledge_base_version",   "Current Delta-Lake KB version")
...
Do not give one-word answers. Do not repeat the question.
...
def debug_memory():
    data = collection.get()
    return {"stored_queries": data["ids"], "stored_answers": data["documents"]}
